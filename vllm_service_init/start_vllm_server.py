...
The key changes from the original script start_vllm_server.py:40-93 :

Moved all initialization inside if __name__ == '__main__': - The tokenizer, model, sample_params, stop_event, pause_event, and idle_thread are now initialized only when the script runs as main, not when imported by child processes
Moved gpu_idle_worker function definition - Placed inside the main guard since it references stop_event and pause_event
Flask app and routes remain at module level - Only the function definitions stay outside the guard, as they need to be accessible to the Flask framework
This structure matches the pattern used in question_generate/question_generate.py question_generate.py:47-124 , which properly wraps vLLM initialization in a main function.
